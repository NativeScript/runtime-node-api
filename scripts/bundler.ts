...
await copy(
  new URL("../packages/macos/build/macos/NativeScript.node", import.meta.url),
...
await $`../deno/target/release/deno compile -A --unstable --no-check -o ${`./${app.name}.app/Contents/MacOS/${app.name}`} ${app.entryPoint}`;
